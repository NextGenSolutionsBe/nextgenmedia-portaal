 ...
 *
 * LEZEN is altijd toegestaan. SCHRIJVEN (content inplannen en invullen) zit in
 * schrijven.mjs en staat standaard UIT: elke ingang moet er expliciet om vragen.
 * De online route doet dat bewust niet zolang daar alleen een sleutel in het
 * adres staat.
 */

import { SCHRIJF_TOOLS, roepSchrijfTool } from './schrijven.mjs'
...
export async function zoekKlant(sb, naam) {
 ...
 */
export function toolsVoor(magSchrijven) {
  return magSchrijven ? [...TOOLS, ...SCHRIJF_TOOLS] : TOOLS
}

export async function roepTool(sb, naam, args = {}, magSchrijven = false) {
  // Schrijven staat standaard uit. Zo kan een nieuwe ingang nooit per ongeluk
  // schrijfrechten erven; die moet er expliciet om vragen.
  if (SCHRIJF_TOOLS.some((t) => t.name === naam)) {
    if (!magSchrijven) {
      throw new Error(
        `${naam} is hier uitgeschakeld. Deze verbinding mag alleen lezen. ` +
        "Gebruik de lokale connector om content in te plannen of in te vullen.",
      )
    }
    return roepSchrijfTool(sb, await zoekKlant(sb, args?.klant), naam, args)
  }
 ...
 */
export async function behandelBericht(sb, bericht, magSchrijven = false) {
  ...
  try {
    ...
    if (method === 'tools/list') return { jsonrpc: '2.0', id, result: { tools: toolsVoor(magSchrijven) } }
    if (method === 'tools/call') {
      const tekst = await roepTool(sb(), params?.name, params?.arguments ?? {}, magSchrijven)
