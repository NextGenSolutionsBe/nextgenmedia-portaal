...
import { Loader2, Plus, Check, X, Clock, Ban } from 'lucide-react'
...
export function PlanningTab({ personeelId }: { personeelId?: string }) {
  ...
  const items = useMemo<KalItem[]>(() => {
    if (!data) return []
    const uit: KalItem[] = []
    for (const p of data.planning) uit.push({ id: `p${p.id}`, datum: p.datum, start: kortUur(p.start_tijd), eind: kortUur(p.eind_tijd), titel: `${kort(p.personeel_id)}${p.taak ?? p.project ?? 'Werkblok'}`, sub: WERKSTATUS[p.werkstatus]?.label, soort: p.status === 'geannuleerd' ? 'planning_afgewezen' : 'planning', onClick: () => setBlok(p) })
    for (const b of data.beschikbaarheid) { const s = beschikbaarheidSoort(b.status); if (s) uit.push({ id: `b${b.id}`, datum: b.datum, start: kortUur(b.start_tijd), eind: kortUur(b.eind_tijd), titel: `${kort(b.personeel_id)}beschikbaar`, sub: b.opmerking, soort: s, onClick: b.status === 'ingediend' ? () => setBeslis(b) : undefined }) }
    ...
    <Dialoog titel={bestaand ? 'Werkblok' : 'Medewerker inplannen'} onSluit={onSluit} breed>
      <div className="space-y-3">
        ...
        <div className="flex justify-between gap-2 pt-1">
          {bestaand && w.status !== 'geannuleerd' ? <button type="button" disabled={bezig} onClick={annuleer} className="btn-secondary text-red-600"><Ban className="h-4 w-4" />Annuleren</button> : <span />}
