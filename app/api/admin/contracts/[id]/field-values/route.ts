...
// velddefinities (detected_fields). AI is hulpmiddel; de mens beslist.
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Saneren: enkel geldige velden bewaren.
    ...
    const { error } = await admin.from('contracts').update({ detected_fields: fields }).eq('id', id)
