...
export async function GET(req: NextRequest) {
  try {
    ...
    const [{ data: mensen, error }, { data: sessies }, { data: planning }, { data: volgende }, { data: openS }, { data: openB }] = await Promise.all([
      admin.from('personeel').select('id, voornaam, achternaam, email, telefoon, type, functie, afdeling, actief, profielfoto_pad, account_status, startdatum, einddatum, created_at').order('voornaam'),
    ...
    return NextResponse.json({ medewerkers, van, tot, magFinancieel: financieel, magGevoelig: await magGevoelig(persoon) })
  } catch (err) {
